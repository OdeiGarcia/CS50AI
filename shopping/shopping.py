    ...
    with open(filename) as f:
        ...
        for row in reader:
            row_evidence = [
                ...
                visitor_map[row[15]],  # VisitorType
...
def evaluate(labels, predictions):
    ...
    dif = sum(1 for a,b in zip(labels,predictions) if a and b)

    n_true = sum(1 for a in labels if a)
    n_false = len(labels) - n_true

    sens = dif/n_true

    dif2 = sum(1 for a,b in zip(labels,predictions) if not a and not b)
